...
import org.zy.priceResync.handler.JdJsonPriceHandler;
import org.zy.priceResync.srv.NotificationService;

public class JdScaner {
	private final static org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JdScaner.class);
	private JdJsonPriceHandler handler = new JdJsonPriceHandler();
		...
		for (Product p : ps) {
			...
			for(String rece : reces){
				...
				if(noti == null){
					...
					noti.setTitle("Notifications: jd prices changed");
